...
/**
 * A `*Dto` mirrors a wire format, so every field names its JSON key with `@SerialName` — renaming the
 ...
 * `null`.
 */
class DtoSerialNameRule : TuneScoutRule("dto-serial-name") {
    override fun beforeVisitChildNodes(
        ...
        if (ktClass.name?.endsWith(DTO_SUFFIX) != true) return
