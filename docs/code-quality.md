...
| `*.presentation.widget` | package filter | the Glance app widgets, their receivers and their action callbacks: `RemoteViews` plumbing the AppWidget host drives. What they delegate to — `ObserveWidgetState`, `ControlWidgetPlayback` — is measured |
| `SongDownloadService` | class filter | the Media3 `DownloadService` the system starts and stops while songs download: a service no JVM test can build. What it hands the work to — the `DownloadManager` wiring, `SongDownloadTracker`, `DownloadReconciler` — is measured |
| `*_Impl`, `$$serializer`, `ComposableSingletons`, `$DefaultImpls` | class filter | generated by Room, kotlinx.serialization and the Compose compiler. `DefaultImpls` is the shim left beside an interface that has a method body, for Java callers only — the body itself is measured on the interface, so the shim would be a line no Kotlin test can reach |
