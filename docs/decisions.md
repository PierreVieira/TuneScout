...
kept the flow tests idle where swapping the pane in place did not. Nothing is pushed for the
player: it is not a screen the user went to, so it has no back arrow and Back from the tabs does what
it did before.
