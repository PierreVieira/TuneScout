...
| `translations` | `scripts/check_translations.py`: every string in `values` has a `values-pt-rBR`, a `values-es`, a `values-fr` and a `values-de` twin | A `strings.xml` or the script |
