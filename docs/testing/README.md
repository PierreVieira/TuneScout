...
| Caches | A preview written to the media cache and read back with the network gone, a download that still plays once the played previews are cleared, and the artwork cache the app installs | `core/playback/impl/src/androidTest`, `app/src/androidTest` |
| Downloads | Which songs the requests add up to, against a real database; the reconciler and the tracker against fakes; and, end to end with `data:` previews, an album downloaded and removed, a song downloaded from its sheet, and a song liked after the liked songs were downloaded | `core/database/impl/src/androidTest`, `core/playback/impl/src/test`, `app/src/androidTest` |
