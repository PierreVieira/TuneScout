...
| End to end | Launches the real app, replaces the remote data source through Koin: search → player → options → album, search → play → queue a song → the queue screen, and an album opened beside the songs on a wide window, which Back closes from either tab. They pass in portrait and landscape. | `app/src/androidTest` |
