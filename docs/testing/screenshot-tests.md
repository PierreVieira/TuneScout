...
| Test | States |
| --- | --- |
| `PlayerScreenshotTest` | loading, not found, loaded (liked) and not liked, ended repeating the song, shuffled repeating the queue, side by side, compact (loaded and loading) |
...
| `LibraryScreenshotTest` | list, grid, grid at four per row, two-pane (top-bar create action), filtered, empty, search: recent, results and no results |
...
| `ListDetailScreenshotTest` | the songs beside the player in every variant, beside nothing playing, and beside an album: recently played, and search results in every variant |
