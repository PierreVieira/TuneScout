...
Either way, the task also gets an emulator of its own, `TuneScout_<short_description>`, so any
...
In both modes, create an emulator for this task only: `TuneScout_<short_description>`, a copy of
...
Whenever the task needs a device (instrumented tests, benchmarks, a look at the running app), boot
and use this emulator as described in that file's "Boot" and "Run tests on it" sections. Never use
the user's emulators or another task's.
